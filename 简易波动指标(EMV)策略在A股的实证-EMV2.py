...
def get_stock_data(stock_code):
    ...
    stock_data = stock_data[['code', 'open', 'close', 'high', 'low', 'change', 'volume']]
...
# 计算EMV指标并得到信号和仓位
def emv2(stock_data, n1=20, n2=23):
    ...
    # emv等于em的n1日简单移动平均
    df['emv'] = pd.rolling_mean(df['em'], n1)
    # maemv等于emv的n2日简单移动平均
    df['maemv'] = pd.rolling_mean(df['emv'], n2)
    df.dropna(inplace=True)

    # 计算每天的信号
    df.ix[df['emv'] > df['maemv'], 'signal'] = 1
    df.ix[df['emv'] < df['maemv'], 'signal'] = -1
...
for code in stock_code_list[0:]:
    ...
    re.iloc[0:1, :].to_csv('d:/results.csv', mode='a', header=None, index=False)
